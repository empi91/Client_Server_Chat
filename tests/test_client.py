...
class TestClient(unittest.TestCase):
    ...
    def test_input_validation(self, mock_input):
        ...
        mock_input.side_effect = ["recipient", "A" * (config.message.MAX_MESSAGE_LENGTH + 1)]
        
        # Test message too long
        message = self.client.check_input_command("!message")
        self.assertIsInstance(message, ErrorMessage)
        self.assertEqual(message.header, "Error")
        self.assertTrue(f"Message cannot be longar than {config.message.MAX_MESSAGE_LENGTH} characters" in message.text)
    ...
    def test_account_type_selection(self, mock_input):
        ...
        class DummyConnection:
            ...
            def recv(self, size):
                ...
                response_msg = Message("Account_type_update", {"update_status": True}, "Server", "Client")
